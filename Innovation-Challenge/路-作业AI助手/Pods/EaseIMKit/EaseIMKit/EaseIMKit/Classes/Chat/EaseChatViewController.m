...
//    if(_isDebug){
//        EMChatMessage *eTest = [[EMChatMessage alloc]initWithConversationID:@"userai002" body:[[EMTextMessageBody alloc] initWithText:aText] ext:aExt];
//        [self messagesDidReceive:@[eTest]];
//    }
    aText = [aText stringByReplacingOccurrencesOfString:@"\n" withString:@""];

    [self addMessageSend:aText];
//    [self addMessageRe:@"请稍等，AI正在解答.."];
    [self getAccessTK:aText];
...
//        return [dic objectForKey:@"access_token"];
        [self requestQianFan:text andTK:[dic objectForKey:@"access_token"]];
...
-(void)requestQianFan:(NSString *)text andTK:(NSString *)token{
    ...
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"https://aip.baidubce.com/rpc/2.0/ai_custom/v1/wenxinworkshop/chat/eb-instant?access_token=%@",token]]
    ...
    NSData *postData = [[NSData alloc] initWithData:[[NSString stringWithFormat:@"{\n  \"messages\": [{\n      \"role\": \"user\",\n       \"content\": \"%@\"\n   }]\n}",text] dataUsingEncoding:NSUTF8StringEncoding]];
    ...
    completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
      ...
      if (error) {
        ...
        NSLog(@"%@",responseDictionary);
          ...
          dispatch_async(dispatch_get_main_queue(), ^{
              [self addMessageRe:[responseDictionary objectForKey:@"result"]];
          });
...
-(void)addMessageRe:(NSString *)text{
    EMChatMessage *eTest = [[EMChatMessage alloc]initWithConversationID:@"userai002" body:[[EMTextMessageBody alloc] initWithText:text] ext:nil];
    ;
//    EMChatMessage *eTest = [[EMChatMessage alloc]initWithConversationID:@"userai002" body:[[EMTextMessageBody alloc] initWithText:text] ext:nil];*
    EaseMessageModel *md = [[EaseMessageModel alloc] initWithEMMessage:eTest];
    md.direction = EMMessageDirectionReceive;
    [self.dataArray addObject:md];
    [self refreshTableView:true];
    [MBProgressHUD hideHUDForView:self.view animated:true];
}

-(void)addMessageSend:(NSString *)text{
    EMChatMessage *eTest = [[EMChatMessage alloc]initWithConversationID:@"userai002" body:[[EMTextMessageBody alloc] initWithText:text] ext:nil];
    ;
//    EMChatMessage *eTest = [[EMChatMessage alloc]initWithConversationID:@"userai002" body:[[EMTextMessageBody alloc] initWithText:text] ext:nil];*
    EaseMessageModel *md = [[EaseMessageModel alloc] initWithEMMessage:eTest];
