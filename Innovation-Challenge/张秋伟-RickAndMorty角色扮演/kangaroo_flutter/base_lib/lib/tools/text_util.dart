...
class TextUtil {
  ...
  /// 获取get请求网络字符串
  static String? getNetString(Map<String,dynamic> map){
    ...
    if(param.isNotEmpty){
      var netString = param.toString();
      return netString.substring(0,netString.length-1);
    }else{
      return null;
    }
  }
  ...
  ///去除url指定参数
  static String removeUrlParam(String ur, List<String> param){
    String u = ur;
    param.forEach((element) {
      u = u.replaceAll(RegExp("&?${element}=[^&]*"), "");
    });
    return u;
  }

  static String removeAllUrlParam(String ur){
    var index = ur.indexOf("?");
    if (index != -1) {
      return ur.substring(0, index);
    }
    return ur;
  }
}
