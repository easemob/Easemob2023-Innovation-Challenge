<template>
	<view class="page">
		<view class="infor">
			...
			<view class="infoBox">
				<text class="doName">{{doctorInfo.user.nikename}}</text>
				<text class="intro">主任医生/教授 | {{doctorInfo.specialization}}</text>
				<view class="bottonBox">
					<text>今日询问11次数</text>
					<uni-rate size="18" :value="doctorInfo.rating" />
				</view>
...
<script>
	export default {
		...
		async onLoad(option) {
			console.log("查看数据",option)
			
			this.getDoctorInfo(option.doctorId)
		...
		methods: {
			initMsg(name){
				...
				var msg={
					...
					userId:"123123",
					userName:name,
					headImage:'../../static/index/doctorImage.png',
					textData:"你好，请问有什么能帮助你的",
					isSelf:false
				}
			...
			getDoctorInfo(did) {
				var _this=this
				this.$requestUtil.get('/medical/doctor/detail/'+did)
					.then((res) => {
						// 处理请求成功的响应
						if(res.code==200){
							_this.doctorInfo=res.data
							_this.initMsg(res.data.user.nikename)
						}
