...
- (void)autoChat {
    NSString *text = [self getLatestOtherTextMessage];
    if (text.length == 0) {
        text = @"需要你的协助";
    }
    text = [@"请你用中文回复这句话：" stringByAppendingString:text];
    OpenAISwiftHelper *helper = [OpenAISwiftHelper new];
    [helper sendGTPWithQuery:text completion:^(NSString * _Nullable answer, NSError * _Nullable) {
        dispatch_async(dispatch_get_main_queue(), ^{
            NSString *msg = answer;
            if(msg.length == 0) {
                msg = @"很抱歉，我没理解，请重新操作";
            ...
            [self.chatController sendTextAction:msg ext:nil];
        });
    }];
}
