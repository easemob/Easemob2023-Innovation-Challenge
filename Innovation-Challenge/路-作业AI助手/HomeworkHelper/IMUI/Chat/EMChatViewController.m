...
@interface EMChatViewController ()<EaseChatViewControllerDelegate, EMChatroomManagerDelegate, EMGroupManagerDelegate, EMMessageCellDelegate, EMReadReceiptMsgDelegate,ConfirmUserCardViewDelegate,EMMultiDevicesDelegate,EMUrlPreviewMessageCellDelegate,GADFullScreenContentDelegate>{
    UIView *titleView;
}
...
-(void)viewDidAppear:(BOOL)animated{
    ...
    [s objectForKey:@"Timer"];
    if(!s){
        [s setObject:@"1" forKey:@"Timer"];
        [s synchronize];
    }else{
    ...
    if(_scanStr){
//        [_chatController chatBarSendMsgAction:_scanStr];
        [_chatController sendTextAction:_scanStr ext:[NSDictionary dictionary]];
        _scanStr = nil;
    }
...
{
    titleView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width * 06, 40)];
